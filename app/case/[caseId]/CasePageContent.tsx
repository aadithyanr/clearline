...
const SEVERITY_CONFIG = {
  critical: { color: '#ef4444', bg: 'bg-red-500', label: 'CRITICAL', text: 'text-red-600', border: 'border-red-200', glow: 'shadow-red-500/20' },
  urgent:   { color: '#f97316', bg: 'bg-orange-500', label: 'URGENT',   text: 'text-orange-600', border: 'border-orange-200', glow: 'shadow-orange-500/20' },
  'non-urgent': { color: '#22c55e', bg: 'bg-green-500', label: 'NON-URGENT', text: 'text-green-600', border: 'border-green-200', glow: 'shadow-green-500/20' },
};
...
export default function CasePageContent({ caseId, initialData, initialError }: { caseId: string; initialData: EmergencyCase | null; initialError: string | null }) {
  ...
  useEffect(() => {
    ...
    // Initial load
    if (!initialData) loadCase(false);

    // Live poll every 5s so dispatcher overrides reflect instantly on this page
  ...
  if (error) return (
    <div className="fixed inset-0 bg-slate-50 flex flex-col items-center justify-center gap-4 px-6 text-center">
      <div className="w-14 h-14 rounded-full bg-red-100 border border-red-200 flex items-center justify-center text-red-500 text-2xl mb-2">✕</div>
      <h1 className="text-slate-800 font-bold text-xl">Case Not Found</h1>
  ...
  if (loading || !caseData) return (
    <div className="fixed inset-0 bg-slate-50 flex items-center justify-center">
      <div className="text-center">
        <div className="w-10 h-10 border-2 border-slate-200 border-t-sky-500 rounded-full animate-spin mx-auto mb-4" />
        <p className="text-slate-500 text-xs font-bold tracking-widest uppercase">Loading {caseId}...</p>
      </div>
    </div>
  );
  ...
  const rec = caseData.assignedHospital as any;
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 pt-safe-top pt-4 px-4 flex items-center justify-between">
        <div className="flex items-center gap-2 bg-white/80 backdrop-blur-xl border border-slate-200 rounded-full px-3 py-1.5 shadow-sm">
          <div className={`w-2 h-2 rounded-full ${sev.bg} ${caseData.status === 'en_route' ? 'animate-pulse' : ''}`} />
          <span className="text-slate-800 text-xs font-bold tracking-wider">{caseId}</span>
        </div>
        <div className={`flex items-center gap-1.5 bg-white/80 backdrop-blur-xl border ${sev.border} rounded-full px-3 py-1.5 shadow-sm`}>
      ...
      <div className="relative z-10 mt-auto mx-3 mb-4">
        <div className="bg-white/95 backdrop-blur-2xl border border-slate-200 rounded-3xl overflow-hidden shadow-xl">
